...
def subscribe_push():
    ...
    try:
        ...
        db.session.rollback()
        # Гонка — endpoint появился между check и insert, обновляем
        existing = PushSubscription.query.filter_by(endpoint=endpoint).first()
        if existing:
            existing.user_id = user_id
            existing.p256dh_key = p256dh
...
@notifications_bp.route('/debug-scheduler', methods=['GET'])
@jwt_required()
def debug_scheduler():
    """Диагностика планировщика напоминаний"""
    user_id = int(get_jwt_identity())
    now_utc = datetime.now(timezone.utc)

    prefs = NotificationPreference.query.filter_by(user_id=user_id).first()
    if not prefs:
        return jsonify({'error': 'Нет настроек уведомлений'})

    # Timezone пользователя
    user_tz_str = getattr(prefs, 'timezone', None) or 'UTC'
    try:
    ...
    return jsonify({
        ...
        'user_localtime': user_now.strftime('%Y-%m-%d %H:%M:%S'),
        'user_hhmm': user_now.strftime('%H:%M'),
        'meal_reminders_enabled': prefs.meal_reminders,
        'meal_times': {
            'breakfast': prefs.breakfast_time,
            'lunch': prefs.lunch_time,
            'dinner': prefs.dinner_time,
        },
...
@notifications_bp.route('/trigger-check', methods=['POST'])
@jwt_required()
def trigger_scheduler_check():
    """Вручную запустить проверку напоминаний (для отладки)"""
    from services.scheduler_service import check_meal_reminders
    check_meal_reminders(current_app._get_current_object())
    return jsonify({'message': 'Проверка выполнена, смотри логи сервера'})
