...
import {
  ...
  ClipboardList, BarChart2, BookOpen, ScrollText,
  ArrowLeft, WandSparkles,
} from 'lucide-react';
...
const NAV = [
  ...
  { to: '/admin/forms',                   label: 'Формы',         icon: WandSparkles },
  ...
  { to: '/admin/logs',                    label: 'Журнал аудита', icon: ScrollText },
];
          ...
          <div className="px-3 py-3 border-t border-primary/8 flex-shrink-0">
            <button onClick={() => navigate('/')}
              className="flex items-center gap-3 px-3 py-2.5 w-full rounded-xl text-sm
                text-primary/40 hover:bg-bg hover:text-accent transition-colors duration-150">
              <ArrowLeft size={16} />
              На портал
            </button>
          </div>
