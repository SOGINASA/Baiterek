...
from models import db, User, UserGoals, NotificationPreference
...
def login():
    ...
    try:
        ...
        # Синхронизируем таймзону из браузера (если передана)
        client_tz = data.get('timezone')
        if client_tz:
            from zoneinfo import ZoneInfo, ZoneInfoNotFoundError
            try:
                ZoneInfo(client_tz)  # валидация
...
def complete_onboarding():
    ...
    try:
        ...
        # Единоразовый расчёт нормы калорий на основе профиля
        nutrition = calculate_nutrition_goals(user)
        if nutrition:
            goals = UserGoals.query.filter_by(user_id=user.id).first()
            if not goals:
                goals = UserGoals(user_id=user.id)
                db.session.add(goals)
            goals.calories_goal = nutrition['calories_goal']
            goals.protein_goal = nutrition['protein_goal']
            goals.fats_goal = nutrition['fats_goal']
            goals.carbs_goal = nutrition['carbs_goal']
