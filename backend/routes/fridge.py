...
from services.receipt_scan_service import scan_receipt, scan_barcode
...
# ==================== Сканирование чеков и штрихкодов ====================

@fridge_bp.route('/scan-receipt', methods=['POST'])
@jwt_required()
def scan_receipt_endpoint():
    """Распознать продукты с фото чека через g4f vision"""
    ...
    try:
        products = scan_receipt(image_bytes, content_type)
        return jsonify(products)
    except Exception as e:
        logger.error(f"Receipt scan failed: {e}")
        return jsonify({'error': 'Не удалось распознать чек. Попробуйте ещё раз.'}), 500


@fridge_bp.route('/scan-barcode', methods=['POST'])
@jwt_required()
def scan_barcode_endpoint():
    """Распознать продукт по фото штрихкода через g4f vision"""
    if 'file' not in request.files:
        return jsonify({'error': 'Файл не прикреплён'}), 400
    ...
    try:
        product_info = scan_barcode(image_bytes, content_type)
        return jsonify(product_info)
    except Exception as e:
        logger.error(f"Barcode scan failed: {e}")
        return jsonify({'error': 'Не удалось распознать штрихкод. Попробуйте ещё раз.'}), 500


CATEGORY_MAP = {
    'en:dairies': 'dairy', 'en:meats': 'meat', 'en:vegetables': 'vegetables',
    'en:fruits': 'fruits', 'en:breads': 'bakery', 'en:beverages': 'drinks',
    'en:cereals-and-potatoes': 'grains', 'en:frozen-foods': 'frozen',
    'en:snacks': 'snacks', 'en:milk': 'dairy', 'en:cheeses': 'dairy',
    'en:yogurts': 'dairy', 'en:fish': 'meat', 'en:seafood': 'meat',
}


def _off_category_to_local(categories_tags):
    """Map Open Food Facts category tags to local category."""
...
@fridge_bp.route('/barcode-lookup', methods=['GET'])
@jwt_required()
def barcode_lookup():
    """Поиск продукта по штрихкоду через Open Food Facts"""
    code = request.args.get('code', '').strip()
    if not code:
        return jsonify({'error': 'Параметр code обязателен'}), 400
    ...
    try:
        ...
        if resp.status_code != 200:
            return jsonify({'found': False, 'code': code}), 200
        ...
        category = _off_category_to_local(product.get('categories_tags', []))
