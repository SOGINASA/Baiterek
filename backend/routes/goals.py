...
@goals_bp.route('reset', methods=['POST'])
@jwt_required()
def reset_goals():
    """Сбросить цели на дефолтные значения модели"""
    try:
        user_id = int(get_jwt_identity())
        goals = UserGoals.query.filter_by(user_id=user_id).first()
        ...
        # Создаём заново с дефолтами модели
        goals = UserGoals(user_id=user_id)
        db.session.add(goals)
        db.session.commit()

        return jsonify({
            'message': 'Цели сброшены',
            'goals': goals.to_dict()
        })

    except Exception as e:
        db.session.rollback()
        print(f"Ошибка сброса целей: {e}")
        return jsonify({'error': 'Ошибка при сбросе'}), 500
