...
from models import db, Meal, MealIngredient, User, UserGoals
from datetime import datetime, timezone
from utils.validators import parse_date, parse_date_range
from services.meal_advice_service import generate_personalized_advice
import json
import logging
import os
import requests as http_requests

# Импортируем отложенно чтобы избежать циклических зависимостей
def _trigger_challenge_updates(user_id):
    try:
        from routes.challenges import update_challenge_progress, update_pair_streaks
        update_challenge_progress(user_id)
        update_pair_streaks(user_id)
    except Exception as e:
        logger.warning(f"[challenges update] {e}")
...
def create_meal():
    ...
    try:
        ...
        # Получаем персонализированный совет, если не передан
        ai_advice = data.get('ai_advice')
        if not ai_advice and data.get('calories', 0) > 0:
            try:
                ...
                if user:
                    ...
                    user_profile = {
                        ...
                        'diet_notes': user.diet_notes or '',
                        'health_flags': json.loads(user.health_flags) if user.health_flags else [],
                        'health_notes': user.health_notes or ''
                    }
                    
                    # Подготавливаем данные целей
                    goals_data = {
                        'calories_goal': goals.calories_goal or 2000,
                        'protein_goal': goals.protein_goal or 100,
                        'carbs_goal': goals.carbs_goal or 200,
                        'fats_goal': goals.fats_goal or 70,
                        'activity_level': goals.activity_level or 'moderate',
                        'goal_type': goals.goal_type or 'maintain'
                    }
                    
                    # Подготавливаем данные о приёме пищи
                    meal_data = {
                        'name': data.get('name'),
                    ...
                    ai_advice = generate_personalized_advice(user_profile, goals_data, meal_data, language)
            except Exception as e:
        ...
        # Обновляем прогресс челленджей и парных стриков в фоне
        _trigger_challenge_updates(user_id)
...
@meals_bp.route('/generate-advice', methods=['POST'])
@jwt_required()
def generate_meal_advice():
    """
    Generate personalized AI advice for a meal based on user's profile and goals.
    ...
    Request body:
    {
        ...
        "carbs": 40,
        "fats": 20,
        "tags": [" chicken", "rice"]  // optional
    }
    
    Returns:
    {
        "advice": "Персонализированный совет от ИИ..."
    }
    """
    try:
        user_id = int(get_jwt_identity())
        ...
        # Генерируем персонализированный совет
        advice = generate_personalized_advice(user_profile, goals_data, meal_data, language)
        ...
        return jsonify({'error': 'Не удалось сгенерировать совет'}), 500


@meals_bp.route('/predict/with-nutrition', methods=['POST', 'OPTIONS'])
def analyze_photo():
    if request.method == 'OPTIONS':
        return '', 200
    """Прокси для анализа фото через predicts-сервер (обход CORS)"""
    if 'file' not in request.files:
        return jsonify({'error': 'Файл не передан'}), 400

    file = request.files['file']
    predicts_url = os.environ.get(
        'PREDICTS_URL',
        'https://korolevst.supertest.beast-inside.kz/food_predict'
    )

    try:
        resp = http_requests.post(
            f'{predicts_url}/predict/with-nutrition',
            files={'file': (file.filename, file.stream, file.content_type)},
            timeout=30,
        )
        return jsonify(resp.json()), resp.status_code
    except http_requests.exceptions.Timeout:
        return jsonify({'error': 'Сервис анализа не ответил вовремя'}), 504
    except Exception as e:
        logger.error(f"Ошибка проксирования analyze-photo: {e}", exc_info=True)
        return jsonify({'error': 'Ошибка сервиса анализа'}), 502